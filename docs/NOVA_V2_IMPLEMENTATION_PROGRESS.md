...
| `Milestone 4` | Conversation and realtime voice | `74%` | Conversation and realtime voice foundations are real and event-linked, the websocket voice path now supports optional chunked input and output transport, the main avatar now accepts progressive PCM segments instead of waiting for full-turn WAV buffering, event-linked follow-up is reachable from both the active popup and recent-event controls, and the coordinator now persists, incrementally merges, and explicitly clears sanitized per-session home context across later text and voice turns, with real cross-surface milestone coverage for carry-over, one-shot event overlay, and explicit clear behavior, but broader coordinator validation outside the mocked milestone harness and realtime-provider work are still pending. |
...
| `Overall` | Weighted V2 roadmap progress | `74%` | Strong foundation and interaction model, with major architecture and productization milestones still incomplete. |
...
- broader end-to-end validation of chat and voice through the new coordinator beyond the current focused milestone slices and mocked-service harness
