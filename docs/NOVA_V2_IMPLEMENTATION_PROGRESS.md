...
| `Milestone 6` | Admin, metrics, and productization | `33%` | Parallel runtime, runtime-path work, and installer groundwork exist, and the V2 admin motion archive now exposes, filters, groups, and summarizes canonical event metadata, but the broader admin event timeline and productization work are still mostly ahead. |
| `Overall` | Weighted V2 roadmap progress | `50%` | Strong foundation and interaction model, with major architecture and productization milestones still incomplete. |
...
| Ticket | Status | Notes |
| --- | --- | --- |
| `V2-050` | `partial` | The V2 admin motion archive now exposes canonical event type, id, and source metadata in the API and review UI, supports event-type filtering, can pivot the archive view by event type, and now summarizes visible event mix, but there is still no dedicated event timeline or cross-event admin view. |
