...
- [conversation_service.py](/opt/avatar-server/avatar_backend/services/conversation_service.py) now also persists a sanitized per-session home-context snapshot, so later text and voice turns can reuse the latest structured home context without every caller resending it
