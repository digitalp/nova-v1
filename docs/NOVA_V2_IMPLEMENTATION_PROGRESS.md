...
| `Milestone 4` | Conversation and realtime voice | `79%` | Conversation and realtime voice foundations are real and event-linked, the websocket voice path now supports optional chunked input and output transport, the main avatar now accepts progressive PCM segments instead of waiting for full-turn WAV buffering, event-linked follow-up is reachable from both the active popup and recent-event controls, and the coordinator now persists, incrementally merges, and explicitly clears sanitized per-session home context across later text and voice turns, with nested dict/list context flattened into stable dotted keys and event follow-up context able to stay active for one additional turn across chat and voice surfaces, but broader coordinator validation outside the current mixed harness and realtime-provider work are still pending. |
...
| `Overall` | Weighted V2 roadmap progress | `77%` | Strong foundation and interaction model, with major architecture and productization milestones still incomplete. |
