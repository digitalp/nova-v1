...
| `Milestone 4` | Conversation and realtime voice | `86%` | Conversation and realtime voice foundations are real and event-linked, the websocket voice path now supports optional chunked input and output transport, the main avatar now accepts progressive PCM segments instead of waiting for full-turn WAV buffering, event-linked follow-up is reachable from both the active popup and recent-event controls, and the coordinator now persists, incrementally merges, and explicitly clears sanitized per-session home context across later text and voice turns, with nested dict/list context flattened into stable dotted keys, event follow-up context able to stay active for one additional turn across chat and voice surfaces, the streamed head-backed path coalescing adjacent PCM chunks before each `speakAudio` call to reduce visible chunk-boundary lipsync resets, and `RealtimeVoiceService` now has an injectable adapter seam with real `/ws/voice` coverage for streamed input, streamed output, interruption behavior, combined context carry-over, and custom provider-adapter routing, but broader realtime-provider product work is still pending. |
...
| `Overall` | Weighted V2 roadmap progress | `83%` | Strong foundation and interaction model, with major architecture and productization milestones still incomplete. |
...
| `V2-031` | `in_progress` | `RealtimeVoiceService` exists and V2 now supports per-session turn state, interruption, `turn_started`, `turn_finished`, `turn_interrupted`, `audio_start`, turn-aware client playback handling, optional streamed audio input buffering with explicit start/commit/cancel frames, negotiated output formats for streamed audio, progressive PCM playback on both the fallback audio path and the main head-backed avatar path, and an injectable provider-adapter seam with real websocket milestone coverage for streamed input commit, streamed input cancel discard behavior, same-socket interruption takeover, streamed-output interruption behavior, combined context carry-over, and custom adapter routing. Remaining work: richer lipsync on streamed chunks, deeper conversation integration, and fuller provider-native backends on top of the new adapter seam. |
...
- [realtime_voice_service.py](/opt/avatar-server/avatar_backend/services/realtime_voice_service.py) now resolves an optional app-provided realtime voice adapter, while preserving the default STT/conversation/TTS orchestration path behind a compatibility adapter
...
- [test_realtime_voice_service.py](/opt/avatar-server/tests/test_realtime_voice_service.py) now also verifies that an app-provided realtime voice adapter can replace the default STT/conversation/TTS pipeline
- [test_voice_milestone.py](/opt/avatar-server/tests/test_voice_milestone.py) now exercises websocket capability negotiation, streamed output metadata, streamed input commit plus cancel semantics, same-socket interruption takeover, streamed-output interruption behavior, combined home-context plus event-overlay carry-over, and custom realtime adapter routing through the real `/ws/voice` route using `TestClient`
